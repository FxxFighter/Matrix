...
void sort_rows(vector<vector<int>>& matrix) {
    for (auto& row : matrix) {
        sort(row.begin(), row.end());  // مرتب‌سازی هر سطر
    }
}

// مرتب‌سازی ستون‌های ماتریس
void sort_columns(vector<vector<int>>& matrix) {
    ...
    for (int j = 0; j < cols; j++) {
        ...
        // قرار دادن دوباره ستون مرتب‌شده در ماتریس
        for (int i = 0; i < rows; i++) {
            matrix[i][j] = column[i];
        }
    }
...
int main() {
    ...
    // مرتب‌سازی سطرها
    sort_rows(matrix1);
    cout << "\nماتریس اول پس از مرتب‌سازی سطرها:\n";
    print_matrix(matrix1);

    // مرتب‌سازی ستون‌ها
    sort_columns(matrix1);
    cout << "\nماتریس اول پس از مرتب‌سازی ستون‌ها:\n";
    print_matrix(matrix1);
    ...
    // جمع ستون‌ها
    cout << "\nجمع عناصر ستون‌ها:\n";
    vector<int> colSums = column_sum(matrix1, rows1, cols1);
    for (int sum : colSums) cout << sum << "\t";
    cout << endl;
